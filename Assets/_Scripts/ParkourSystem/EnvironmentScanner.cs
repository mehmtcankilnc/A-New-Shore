...
public class EnvironmentScanner : MonoBehaviour
{
    ...
    [SerializeField] private LayerMask pushObstacleLayer;
    ...
    public ObstacleHitData ObstacleCheck()
    {
        ...
        Debug.DrawRay(forwardOrigin, transform.forward * forwardRayLength, hitData.forwardHitFound ? Color.blue : Color.white);
        ...
        if (hitData.forwardHitFound)
        {
            ...
            Debug.DrawRay(heightOrigin, Vector3.down * heightRayLength, hitData.heightHitFound ? Color.blue : Color.white);
        }

        return hitData;
    }

    public bool PushObstacleCheck(out RaycastHit pushHit)
    {
        var forwardOrigin = transform.position + forwardRayOffset;

        Debug.DrawRay(forwardOrigin, transform.forward * forwardRayLength, Color.white);

        return Physics.Raycast(forwardOrigin, transform.forward, out pushHit, forwardRayLength, pushObstacleLayer);
    }
    ...
    public bool ClimbLedgeCheck(Vector3 moveDir, out RaycastHit ledgeHit)
    {
        ...
        for (int i = 0; i < 6; i++)
        {
            Debug.DrawRay(origin + offset * i, moveDir);
            if (Physics.Raycast(origin + offset * i, moveDir, out RaycastHit hit, climbLedgeRayLength, climbLedgeLayer))
            {
                ledgeHit = hit;
                return true;
            }
